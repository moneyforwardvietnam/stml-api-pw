...
export default class BillingsEndpoint extends AuthedRequest {
    ...
    create = async () => {
        ...
        const data = {
            "department_id": this.sharedData.getContext(AppId.APP_DEPARTMENT_ID),
        ...
        this.sharedData.setContext(AppId.APP_BILLING_ID, (await response.json()).id);
        return response;
    }
    ...
    createItem = async () => {
        ...
        this.sharedData.setContext(AppId.APP_ITEM_ID, (await response.json()).id);
        return response;
    }
    ...
    delete = async () => {
        await this.initContext();
        return await this.deleteById(this.sharedData.getContext(AppId.APP_BILLING_ID));
    }


    deleteItem = async () => {
        await this.initContext();
        const path = `${ITEMS_EP}/${this.sharedData.getContext(AppId.APP_ITEM_ID)}`;
        return await this.requestSender(HttpMethod.DELETE, path);
    }
}